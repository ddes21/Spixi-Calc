...
// --- Spixi Mini-App: Close/Back handler ---
(function () {
  function tryCall(fn) {
    try { return fn(); } catch (_) { /* no-op */ }
  }

  async function closeMiniApp() {
    // 1) If the Spixi Mini-App SDK is injected, prefer its UI close/exit
    //    (names can vary across builds; try a few common shapes)
    if (window.SpixiApp && typeof SpixiApp.close === 'function') return SpixiApp.close();
    if (window.Spixi && typeof Spixi.close === 'function')        return Spixi.close();
    if (window.Spixi && Spixi.ui && typeof Spixi.ui.close === 'function') return Spixi.ui.close();
    if (window.spixi && typeof spixi.closeMiniApp === 'function') return spixi.closeMiniApp();

    // 2) PostMessage bridge (some WebView bridges listen for this)
    tryCall(() => window.parent && window.parent.postMessage({ type: 'spixi-ui', action: 'close' }, '*'));

    // 3) Custom URL scheme (Spixi may intercept)
    tryCall(() => { window.location.href = 'spixi://close'; });

    // 4) Fallbacks: go back in history, then try window.close()
    if (window.history.length > 1) {
      return window.history.back();
    }
    tryCall(() => window.close());
  }

  // wire the button
  const btn = document.getElementById('spixi-close');
  if (btn) btn.addEventListener('click', closeMiniApp);

  // optional niceties:
  // - ESC twice quickly to close
  let escTs = 0;
  window.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      const now = Date.now();
      if (now - escTs < 500) closeMiniApp();
      escTs = now;
    }
  }, { passive: true });

  // - Android back-swipe / back button sometimes maps to history.back().
  //   If your app is a single page with no history, the fallback will trigger.
})();
